...
class LoginController extends Controller
{
    ...
    public function migraPaisDepàrtameto(Request $request){
        ...
        $data = json_decode($jsonPaises, true);
        ...
        $jsonStates = '{
            ...
            ';
        $dataStates = json_decode($jsonStates, true);
        $States = $dataStates['states'];

        // dd($filteredArray);

        foreach ($countries as $country) {
            echo "<h2>".$country['id'] . ': ' . $country['name'] . "</h2><br>";
            $pais                       = new Pais();
            $pais->pais                 = $country['name'];
            $pais->estado               = 1;
            $pais->usuario_creacion     = 1;
            $pais->save();

            $idToFilter = $country['id'];

            $filteredArray = array_filter($States, function($element) use ($idToFilter) {
                return isset($element['id_country']) && $element['id_country'] === $idToFilter;
            });

            foreach($filteredArray as $de){
                echo "   -->".$de['id_country']." : ".$de['name']."<br>";
                $departamento                   = new Departamento();
                $departamento->idPaises         = $pais->idPaises;
                $departamento->departamento     = $de['name'];
                $departamento->estado           = 1;
                $departamento->usuario_creacion = 1;
                $departamento->save();
            }
